...
def dynamicUpdate():
    ...
    for i in range(len(smartHome.devices)):
        ...
        # Device label is added to this list so that the config can be changed to reflect the GUI changes
        ...
        def customWindow(d=device):
            ...
            def changeConsumption(d=device):
                d.setConsumptionRate(consumptionEntry.get())
                dynamicUpdate()

            consumptionConfirm = Button(customWindow, text="Confirm", command=changeConsumption)
            ...
            if isinstance(d, SmartWashingMachine):
                ...
                for index in range(len(d.getWashModes())):
                    ...
                    # Anonymous function used to update the value of a device label as well as update the GUI
